...
from genover import GeometricSMOTE
...
genover_gbc = make_pipeline(GeometricSMOTE(random_state=RANDOM_STATE), 
                           DecisionTreeClassifier(random_state=RANDOM_STATE))

scoring_name = 'Geometric Mean Score'
validation_curve_info = generate_validation_curve_info(genover_gbc, X, y, range(1, 8), "geometricsmote__k_neighbors", SCORER)
plot_validation_curve(validation_curve_info, scoring_name, 'K Neighbors')

validation_curve_info = generate_validation_curve_info(genover_gbc, X, y, np.linspace(-1.0, 1.0, 9), "geometricsmote__truncation_factor", SCORER)
plot_validation_curve(validation_curve_info, scoring_name, 'Truncation Factor')

validation_curve_info = generate_validation_curve_info(genover_gbc, X, y, np.linspace(0.0, 1.0, 5), "geometricsmote__deformation_factor", SCORER)
plot_validation_curve(validation_curve_info, scoring_name, 'Deformation Factor')

validation_curve_info = generate_validation_curve_info(genover_gbc, X, y, ['minority', 'majority', 'combined'], "geometricsmote__selection_strategy", SCORER)
